...
static void draw_stroke(GCanvas* canvas, GPoint p0, GPoint p1, float width, bool roundCap,
                        const GPaint& paint) {
    GPath path;
    canvas->final_addStrokedLine(&path, p0, p1, width, roundCap);
...
static void final_triangle(GCanvas* canvas) {
    GPoint pts[] = {
        { 10, 10 }, { 500, 50 }, { 50, 400 }
    };
    GColor colors[] = {
    ...
    auto sh = canvas->final_createTriangleGradient(pts, colors);
    if (sh) {
        canvas->drawConvexPolygon(pts, 3, GPaint(sh.get()));
    }
}
